...
从 GitHub Releases 下载最新 PicoOnebot APK 并安装。手表 QQ 底包只提供 `armeabi-v7a` 原生库，因此设备必须支持 32 位应用；纯 64 位且没有厂商转译层的机型无法直接安装。没有合适的旧手机时，Windows 和 macOS 可以使用 Android 模拟器；Linux 或服务器可以使用项目提供的 Docker 方案，容器内置 Android 环境与 ARM 转译，但资源开销会高于 APK 直装。启动后授予必要权限。如果直接在运行 PicoOnebot 的手机浏览器操作，可访问 `http://127.0.0.1:6099`；如果从同一局域网的其它设备访问，则使用 `http://<手机IP>:6099`。初始密码是 `picopico`，首次登录后需要修改。进入扫码页，用另一个已登录 QQ 的设备扫码，让机器人账号上线。
...
在 PicoOnebot WebUI 的“网络配置”中添加“反向 WebSocket”。目标地址填写机器人框架实际提供的 `ws://` 或 `wss://` 地址，可以是局域网 IP，也可以是公网可访问的域名或 IP；外部机器人不要填写 `127.0.0.1`。对方若设置了 Token，PicoOnebot 中填写完全一致的值；未设置则留空。保存后网络配置会热重载，不需要重启 QQ，并会按配置自动重连。
