...
网络页和文件管理器编辑当前生效的 `config.json` 使用相同的热更新逻辑：保存后重建 OneBot 监听及全部启用的反向 WebSocket，并更新 HTTP 上报目标。现有 OneBot 连接会短暂断开，不需要重启 QQ。文件编辑也会校验网络配置；端口被占用等监听错误会在保存结果和日志中提示。

多个启用的 WebSocket 客户端均接收事件；动作响应只返回发起请求的连接。各连接独立排队发送，慢连接队列满后会断开，不阻塞其他连接。修改 WebUI 监听地址、端口或进程启动参数仍可能需要重启 QQ。
