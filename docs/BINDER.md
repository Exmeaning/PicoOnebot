...
PicoOnebot 容器运行基于 Redroid 的 Android 环境，依赖宿主机内核提供的 **Binder IPC** 支持。
如果宿主机内核未启用 Binder 或设备节点权限配置不当，Android 核心系统服务将无法启动。本文档介绍 Binder 机制、环境配置与常见问题排查。
...
| 检测状态 | 说明 | 建议操作 |
| --- | --- | --- |
| **binderfs 可用** | 内核已开启 `CONFIG_ANDROID_BINDERFS` | 直接启动容器，无需映射宿主机设备节点 |
| **设备节点可用** | 宿主机存在 `/dev/binder` 等节点 | 配置节点权限为 0666 并通过 `--device` 映射传入 |
...
Binder 是 Android 系统的核心进程间通信（IPC）机制，由内核驱动（`/dev/binder`、`/dev/hwbinder`、`/dev/vndbinder`）提供。Android 的系统服务（如 `servicemanager`、`surfaceflinger`、`system_server` 等）均依赖 Binder 通信。
...
WSL2 官方内核默认未启用 Binder。若需在 WSL2 中运行 Docker 容器，可选择以下方式：

1. **自定义编译 WSL2 内核**：
   从 [microsoft/WSL2-Linux-Kernel](https://github.com/microsoft/WSL2-Linux-Kernel) 获取源码，在配置中开启：
   ```kconfig
   CONFIG_ANDROID=y
   ...
   ```
   编译生成 `bzImage`，在 Windows 用户目录下的 `%USERPROFILE%\.wslconfig` 中配置：
   ```ini
   [wsl2]
   kernel=C:\\path\\to\\bzImage
   ```
   执行 `wsl --shutdown` 重启生效后重新运行检测脚本。

2. **使用标准 Linux 宿主机**：
   在 KVM 虚拟机或物理机 Linux 系统上运行。

3. **使用 APK 方案**：
   在 Android 模拟器或实体设备中安装运行 APK。

---

## 其他部署方案

若运行环境无法提供 Binder 支持，可采用其他形态部署 PicoOnebot：

- **Android 设备 / 模拟器（APK 安装）**：前往 [GitHub Releases](https://github.com/exmeaning/PicoOnebot/releases) 下载 APK 安装至手机、平板或常见模拟器（如 MuMu、雷电、AVD 等），通过 `http://<设备IP>:6099` 访问管理页面。
- **Waydroid**：在具备 Binder 支持的 Linux 桌面环境中运行。
...
输出 Binder 节点权限、Android 系统服务状态、转译库、端口监听及近期日志。

### 日志查看

```bash
# 查看容器日志（包含 entrypoint 检测与内核转发日志）
...
```bash
docker exec pico-onebot /system/bin/ls -l /dev/binder /dev/hwbinder /dev/vndbinder
# 正常权限应为: crw-rw-rw- (0666)
```
...
**Q: 容器启动后为何无法立即访问 6099 端口？**
A: 首次启动需完成 Android 系统初始化及 APK 安装引导，耗时约 2–5 分钟。`docker ps` 显示状态为 `healthy` 后即可正常访问。

**Q: 为什么必须使用 `--privileged` 参数？**
A: Android 容器运行需要挂载 binderfs、管理设备节点及配置系统命名空间，必须使用特权模式。

**Q: 是否需要配置 `/dev/ashmem`？**
A: 不需要。本镜像基于 Android 13，系统底层使用 `memfd` 机制，不依赖 ashmem 驱动。

**Q: 是否支持部署多个容器实例？**
A: 支持。需为各容器配置独立的容器名称、端口映射及数据持久化卷。在 binderfs 模式下各实例文件系统相互独立。

**Q: 宿主机执行 `chmod 666 /dev/binder` 是否存在安全风险？**
A: 配置 0666 权限后宿主机所有本地用户均可访问 Binder 驱动。在多租户环境中，推荐优先采用 binderfs 方案以保证隔离性。
